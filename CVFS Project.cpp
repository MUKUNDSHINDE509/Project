...
#define MAXINODE 50   //for desiding how many inodes

#define READ 1  //for permission read 
#define WRITE 2  //write permission

#define MAXFILESIZE 1024   //for maximum size of file

#define REGULAR 1  //for two types of file 
#define SPECIAL 2

#define START 0   //for l_seek function
#define CURRENT 1
#define END 2
...
typedef struct inode
{
    ...
    int FileActualSize;
    int FileType;
    char *Buffer;  //for holding file data address
    int LinkCount;
    int ReferenceCount;
    int permission; //1, 23
    struct inode *next;  //for next inode address
...
typedef struct ufdt  //user file descripter table
{
    PFILETABLE ptrfiletable;
}UFDT;

UFDT UFDTArr[MAXINODE];     //UFDT UFDTArr[50];
...
{
    int i = 0;
    while(i<MAXINODE)  //for removing only garbage in the array
    {
...
int main()
{
    ...
    while(1)
    {
        fflush(stdin);  //cleaning os ufdt array
        strcpy(str," ");  //memset can also use here

        printf("\n Marvellous VFS : >");

        fgets(str,80,stdin);  // scanf("%[^'\n']s",str); (file madhun string anun de -kashat,kiti,kuthun)

        count = sscanf(str,"%s %s %s %s",command[0],command[1], command[2],command[3]);//sscanf-string madhal scan karun de
        ...
        else if(count == 3)
        {
            ...
            {
                ret = CreateFile(command[1],atoi(command[2]));  //atoi-assci to integer
                if(ret>=0)
                {
